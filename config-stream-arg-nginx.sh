...
if [[ ! -z "$MIXER_ADDRESS" ]]; then
	sed -i "12i\\            push rtmp://$MIXER_ADDRESS/$MIXER_STREAM_KEY;" /root/nginx-rmtp-cfg-simple.txt
fi

if [[ ! -z "$FACEBOOK_ADDRESS" ]]; then
	sed -i "12i\\            push rtmp://$FACEBOOK_ADDRESS/$FACEBOOK_ADDRESS;" /root/nginx-rmtp-cfg-simple.txt
...
if [[ ! -z "$TWITCH_ADDRESS" ]]; then
	sed -i "12i\\            push rtmp://$TWITCH_ADDRESS/$TWITCH_STREAM_KEY;" /root/nginx-rmtp-cfg-simple.txt
...
if [[ ! -z "$YOUTUBE_ADDRESS" ]]; then
	sed -i "12i\\            push rtmp://$YOUTUBE_ADDRESS/$YOUTUBE_STREAM_KEY;" /root/nginx-rmtp-cfg-simple.txt
